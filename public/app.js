...
let DOCTORS = [
  ...
  },
  {
    ...
    initials: "ММ",
    spec: "Стоматолог-терапевт, лечение каналов",
    exp: 8,
    rating: 4.7,
    reviews: 29,
    price: 1000,
    slot: "Сегодня, 18:00",
    services: ["Лечение кариеса", "Лечение каналов", "Лечение каналов", "Восстановление зуба", "Профилактика"],
...
function loadCurrentUser() {
  try { return JSON.parse(localStorage.getItem('arpidentUser') || 'null'); }
  catch (_) { return null; }
}
function saveCurrentUser(user) {
  CURRENT_USER = user;
  if (user) localStorage.setItem('arpidentUser', JSON.stringify(user));
  else localStorage.removeItem('arpidentUser');
}
function getClinicParts() {
  return new Intl.DateTimeFormat('en-CA', {
    timeZone: 'Europe/Moscow', year: 'numeric', month: '2-digit', day: '2-digit',
    hour: '2-digit', minute: '2-digit', hour12: false
  }).formatToParts(new Date()).reduce((acc, part) => { acc[part.type] = part.value; return acc; }, {});
}
function getClinicTodayISO() {
  const p = getClinicParts();
  return `${p.year}-${p.month}-${p.day}`;
}
function clinicMinutesNow() {
  const p = getClinicParts();
...
function isPastClinicSlot(dateStr, time) {
  if (!dateStr || !time) return false;
  const today = getClinicTodayISO();
  if (dateStr < today) return true;
  if (dateStr > today) return false;
  return timeToMinutes(time) <= clinicMinutesNow();
}
...
function renderTimeSlots() {
  ...
  el.innerHTML = TIME_SLOTS.map(t => {
    ...
    const past = dateStr && isPastClinicSlot(dateStr, t);
    const busy = busyDefault || userBooked || past;
    const selected = bookingState.time === t;
    let cls = 'time-slot';
    if (past) cls += ' past busy';
    else if (busy) cls += ' busy';
    else if (selected) cls += ' selected';
    const label = past ? `${t}<small>прошло</small>` : t;
    return `<div class="${cls}" onclick="selectTime('${t}')">${label}</div>`;
  }).join('');
}
...
function selectTime(t) {
  const dId = bookingState.doctorId;
  const dateStr = bookingState.date;
  if (dateStr && isPastClinicSlot(dateStr, t)) { showToast('Это время уже прошло', 'error'); return; }
...
function showBookingStep(step) {
  ...
  if (step === 4) { renderBookingSummary(); prefillBookingUser(); }
}
...
function prefillBookingUser() {
  ...
  if (nameEl && !nameEl.value) nameEl.value = CURRENT_USER.name || '';
  if (phoneEl && !phoneEl.value) phoneEl.value = CURRENT_USER.phone || '';
...
function renderCabinet() {
  ...
  if (!CURRENT_USER) {
    el.innerHTML = `
      <div class="cabinet-auth">
        <div class="cabinet-auth__visual">
          <div class="auth-orb"></div>
          <h2>Личный кабинет пациента</h2>
          <p>Сначала зарегистрируйтесь или войдите, чтобы видеть свои записи, отменять приём и повторять запись к врачу.</p>
          <div class="auth-benefits">
            <span>История посещений</span><span>Активные записи</span><span>Повторная запись</span>
          </div>
        </div>
        <div class="cabinet-auth__forms">
          <div class="auth-card">
            <h3>Вход</h3>
            <div class="form-group"><label>Телефон или email</label><input class="form-input" id="login-id" placeholder="+7... или email"></div>
            <div class="form-group"><label>Пароль</label><input class="form-input" id="login-pass" type="password" placeholder="Пароль"></div>
            <button class="btn btn--primary btn--full" onclick="loginCabinetUser()">Войти</button>
          </div>
          <div class="auth-card auth-card--light">
            <h3>Регистрация</h3>
            <div class="form-group"><label>Имя и фамилия</label><input class="form-input" id="reg-name" placeholder="Имя Фамилия"></div>
            <div class="form-group"><label>Телефон</label><input class="form-input" id="reg-phone" placeholder="+7 (___) ___-__-__"></div>
            <div class="form-group"><label>Email</label><input class="form-input" id="reg-email" placeholder="mail@example.com"></div>
            <div class="form-group"><label>Пароль</label><input class="form-input" id="reg-pass" type="password" placeholder="минимум 4 символа"></div>
            <button class="btn btn--outline btn--full" onclick="registerCabinetUser()">Зарегистрироваться</button>
          </div>
        </div>
      </div>`;
    return;
  ...
  const bookings = getBookings().filter(b =>
    b.userId === CURRENT_USER.id || b.phone === CURRENT_USER.phone || (CURRENT_USER.email && String(b.email).toLowerCase() === String(CURRENT_USER.email).toLowerCase())
  );
  ...
  el.innerHTML = `
    <div class="cabinet-layout">
      <div class="cabinet-header">
        <div class="cabinet-avatar">${(CURRENT_USER.name || 'Па').slice(0,2).toUpperCase()}</div>
        <div>
          <div class="cabinet-name">${CURRENT_USER.name}</div>
          <div class="cabinet-meta">${CURRENT_USER.phone || ''}${CURRENT_USER.email ? ' · ' + CURRENT_USER.email : ''}</div>
        </div>
        <button class="btn btn--ghost" style="margin-left:auto" onclick="logoutCabinetUser()">Выйти</button>
      </div>
        ...
        <div>
          <div class="cabinet-section">
            ...
            ${active.length
              ...
              : `<div class="empty-state"><h3>Нет активных записей</h3><p>Выберите врача и удобное время.</p><button class="btn btn--primary" onclick="showSection('booking')">Записаться на приём</button></div>`}
          </div>
        </div>
        <div>
          <div class="cabinet-section">
            <h3>История кабинета</h3>
            ${past.length ? past.map(b => `<div class="history-item"><span class="history-item__date">${formatDate(b.date)}</span><div><div class="history-item__text">${b.service}</div><div style="font-size:0.8rem;color:var(--teal-dark)">${b.doctorName}</div></div></div>`).join('') : HISTORY.map(h => `<div class="history-item"><span class="history-item__date">${h.date}</span><div><div class="history-item__text">${h.service}</div><div style="font-size:0.8rem;color:var(--teal-dark)">${h.doctor}</div></div></div>`).join('')}
          </div>
        </div>
      </div>
    </div>`;
...
async function registerCabinetUser() {
  try {
    const user = await apiRequest('/api/users/register', { method: 'POST', body: JSON.stringify({
  ...
  } catch (error) { showToast(error.message || 'Не удалось зарегистрироваться', 'error'); }
}

async function loginCabinetUser() {
  try {
    const user = await apiRequest('/api/users/login', { method: 'POST', body: JSON.stringify({
  ...
  } catch (error) { showToast(error.message || 'Не удалось войти', 'error'); }
}

function logoutCabinetUser() {
